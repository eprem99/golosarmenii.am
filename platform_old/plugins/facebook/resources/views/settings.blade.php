                ...
                <div class="wrapper-content pd-all-20">
                    <div class="form-group @if ($errors->has('settings.facebook_access_token')) has-error @endif">
                        <label class="text-title-field"
                               for="facebook_enable">{{ trans('plugins/facebook::facebook.settings.enable') }}
                        </label>
                    ...
                    @if (!app()->environment('demo') && !config('plugins.facebook.general.use_token'))
                        <div class="form-group @if ($errors->has('settings.facebook_page_id')) has-error @endif">
                            <label for="facebook_page_id" class="text-title-field">{{ trans('plugins/facebook::facebook.settings.fan_page_id') }}</label>
                            ...
                            <div class="text-success">
                                <small>{!! trans('plugins/facebook::facebook.settings.get_fan_page_id_helper', ['link' => Html::link('https://findmyfbid.com', 'https://findmyfbid.com', ['target' => '_blank'])]) !!} </small>
                            </div>
                    ...
                    @if (!app()->environment('demo') && config('plugins.facebook.general.use_token'))
                        ...
                        @if (setting('facebook_access_token') != null && !empty($listPages))
                            <div class="form-group @if ($errors->has('settings.facebook_page_id')) has-error @endif">
                                <label for="facebook_page_id" class="text-title-field">{{ trans('plugins/facebook::facebook.settings.select_page') }}</label>
                                <div class="ui-select-wrapper">
                                    {!! Form::select('settings[facebook_page_id]', $listPages, old('settings.facebook_page_id.', setting('facebook_page_id')), ['class' => 'ui-select', 'id' => 'facebook_page_id']) !!}
